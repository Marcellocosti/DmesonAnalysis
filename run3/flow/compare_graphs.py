...
from plot_style import SetStyle, GetColor 
from plot_utils import Load, GetNPanels
...
for plot in cfg:
    ...
    for inputCfg in plot["input"]:
        ...
        inObj.SetLineColor(GetColor(inputCfg['color']))
        inObj.SetMarkerColor(GetColor(inputCfg['color']))
    ...
    # Define the canvas
    nPanelsX, nPanelsY = GetNPanels(len(panels))
    ...
    for line in plot['opt']['lines']:
        ...
        inputline.SetLineColor(GetColor(line['color']))
        inputline.SetLineWidth(line['thickness'])
        inputline.Draw("same")
        if line.get('legendtag'):
    ...
    if plot['ratio']['enable']:
        ...
        frame = pad.DrawFrame(x1, y1, x2, y2, plot['opt']['title'])
        ...
        if isinstance(inObj, TH1):
            ...
            print('Ratio for type %s is not implemented. Skipping this object', type(inObj))
            continue
    ...
    if plot['relunc']['enable']:
        ...
        for iObj, inObj in enumerate(inObjs):
            if isinstance(inObj, TH1):
                ...
                print('Relative uncertainties for type %s are not implemented. Skipping this object', type(inObj))
                continue
